...
def optimize(filename: str,
    ...
    with open(mc_log_file, 'w') as f:
        f.write('# Temperature = {:.3f}K, fmax={:.3f} eV/Å, fire_max_steps={}, neighbor_cutoff={}\n'.format(temperature, fmax, fire_max_steps, neighbor_cutoff))
        f.write('# iter, energy_before [eV], energy_after [eV], acceptance, time_cost, \n')

    ### Load initial configuration.
    perov = ase.io.read(filename, format='lammps-data', atom_style='atomic')
    ...
    ### Set the optimizer.
    dyn = FIRE(perov, logfile='{}/fire.log'.format(logging_dir)  )
    dyn.run(fmax=fmax, steps=1000)  ## initial relaxation, do not limit the number of steps
